...
class LineItemTypesTest < ActiveSupport::TestCase
  ...
  test 'type description translations work' do
    I18n.with_locale(:en) do
      assert_equal('Engine oil was drained and refilled.',
                   LineItemTypes::GLOBAL.get_type('engineOilChange').description)
      assert_nil(LineItemTypes::GLOBAL.get_type('cabinAirFilterChange').description,
                 'LineItemTypes::Type#description returns non-nil for missing description')
    end
  end
  ...
  test 'english translations exist for every category' do
    I18n.with_locale(:en) do
      LineItemTypes::GLOBAL.all_categories.each do |category|
        assert(I18n.exists?(category.display_name_key),
               "No translation for category #{category.id} (parent #{category.parent&.id})")
      end
    end
  end
  ...
  test 'english translations exist for every type' do
    I18n.with_locale(:en) do
      LineItemTypes::GLOBAL.all_types.each do |type|
        assert(I18n.exists?(type.display_name_key),
               "No translation for type #{type.id} (category #{type.category&.id})")
      end
    end
  end
    ...
    I18n.with_locale(:en) do
      LineItemTypes::GLOBAL.all_types.each do |type|
        type.fields.each do |field|
          assert(I18n.exists?(field.short_display_name_key),
                 "No translation for type #{type.id} field #{field.id} (category #{type.category&.id})")
        end
      end
    end
  end
